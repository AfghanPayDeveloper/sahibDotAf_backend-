...
export const createHall = async (req, res) => {
  ...
  if (!workspaceId || !hallName) {
    return res
      ...
      .json({
        error:
          "Required fields are missing: workspaceId hallName, or description.",
      });
  }
  ...
  try {
    const newHall = new Hall({
      ...
      hallImages: req.files["hallImages"]
        ? req.files["hallImages"].map((file) => `/uploads/${file.filename}`)
        : [],
      isApproved: false,
    });
    ...
    res
      .status(201)
      .json({
        message: "Hall created successfully, awaiting approval",
        hall: newHall,
      });
  } catch (error) {
...
export const deleteHall = async (req, res) => {
  ...
  try {
    ...
    if (userRole === "superadmin") {
      ...
      hall = await Hall.findOne({ _id: id, workspaceId }).populate(
        "workspaceId"
      );
    }
    ...
    const filesToDelete = [hall.hallThumbnailImage, ...hall.hallImages].filter(
      Boolean
    );
    ...
    if (userRole == "superadmin") {
      if (hall.workspaceId) {
        ...
        sendNotification(hall.workspaceId.userId, {
          ...notification.toJSON(),
          from: req.user,
        });
      }
...
export const updateHallPUT = async (req, res) => {
  ...
  if (!workspaceId || !hallName) {
    return res
      .status(400)
      .json({ error: "Required fields are missing: workspaceId, hallName" });
  }
  ...
  try {
    ...
    if (req.files["hallThumbnailImage"]) {
      const oldHallThumbnailImagePath = path.join(
        process.cwd(),
        hall.hallThumbnailImage
      );
    ...
    if (deletedHallImages && Array.isArray(deletedHallImages)) {
      ...
      hall.hallImages = hall.hallImages.filter(
        (img) => !deletedHallImages.includes(img)
      );
    }

    if (req.files["hallImages"]) {
      const newHallImages = req.files["hallImages"].map(
    ...
    if (req.user.role == "superadmin") {
      if (hall.workspaceId) {
        ...
        sendNotification(hall.workspaceId._id, {
          ...notification.toJSON(),
          from: req.user,
        });
      }
