...
router.post('/', authenticate, sanitizeDescription, upload.fields([
  ...
  try {
    ...
    const serviceThumbnailImage = req.files['serviceThumbnailImage']?.[0]?.path.replace(process.cwd(), '');
    const serviceImages = req.files['serviceImages']?.map(file => file.path.replace(process.cwd(), ''));
...
router.patch('/:id', authenticate, upload.fields([
  ...
  try {
    ...
    if (req.files['serviceThumbnailImage']) {
      if (service.serviceThumbnailImage) deleteFiles([service.serviceThumbnailImage]);
      service.serviceThumbnailImage = req.files['serviceThumbnailImage'][0]?.path.replace(process.cwd(), '');
    }

    if (req.files['serviceImages']) {
      deleteFiles(service.serviceImages);
      service.serviceImages = req.files['serviceImages']?.map(file => file.path.replace(process.cwd(), ''));
    }

    service.serviceName = serviceName || service.serviceName;
    service.serviceDescription = serviceDescription ? JSON.parse(serviceDescription) : service.serviceDescription;
...
router.patch('/:id/approve', authenticate, async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: 'Failed to approve service', error: error.message });
  }
});
...
router.put('/:id', authenticate, sanitizeDescription, upload.fields([{ name: 'serviceThumbnailImage', maxCount: 1 }, { name: 'serviceImages', maxCount: 5 }]), async (req, res) => {
  ...
  try {
    ...
    if (deletedServiceImages && Array.isArray(deletedServiceImages)) {
      ...
      service.serviceImages = service.serviceImages.filter((img) => !deletedServiceImages.includes(img));
    }


    if (req.files['serviceImages']) {
      const newServiceImages = req.files['serviceImages'].map(file => `/uploads/${file.filename}`);
...
router.patch('/:id/unapprove', authenticate, async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: 'Failed to unapprove service', error: error.message });
  }
});
